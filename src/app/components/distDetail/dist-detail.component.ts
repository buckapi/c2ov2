...
@Component({
  selector: 'app-dist-detail',
  templateUrl: './dist-detail.component.html',
  styleUrls: ['./dist-detail.component.css']
})
export class DistDetailComponent implements OnInit {
 ...
 loadClientes(){
   this.dataApiService.getClientes(this.yeoman.preview.url).subscribe(response=>{
     this.clientes=response;this.yeoman.clientes=response;
     this.loadClients();
   });
 }
 loadDists(){
 ...
 developerModeOn(){
  this.counterDeveloperMode++;
  if(this.counterDeveloperMode>=5){
  ...
  for (let i=0; i < this.clientes.length;i++){
    for(let j =0;j<this.yeoman.clients.length;j++ ){
      if(this.clientes[i].clcodigo===this.yeoman.clients[j].cliCodigo){
        ...
        this.clientes[i].status="unactive";
      }
